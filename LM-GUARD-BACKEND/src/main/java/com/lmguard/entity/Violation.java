...
public class Violation extends BaseEntity {
    ...
    /** What an inspector has done about this finding since - independent of {@link #status},
     * which is the rule engine's verdict and never changes after analysis. */
    @Enumerated(EnumType.STRING)
    @Column(name = "case_status", nullable = false, length = 30)
    @Builder.Default
    private ViolationCaseStatus caseStatus = ViolationCaseStatus.OPEN;
